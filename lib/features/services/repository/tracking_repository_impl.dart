    ...
    required T Function(Map<String, dynamic>) fromJson,
      ...
      guard(() async {
        ...
        do {
          ...
          if (raw['success'] != true) {
            throw AppException(message: (raw['message'] as String?) ?? 'Request failed');
          }

          final list = raw['data'] as List<dynamic>;
          all.addAll(list.map((e) => fromJson(e as Map<String, dynamic>)));

          final meta = PaginationMeta.fromJson(raw['meta'] as Map<String, dynamic>);
  ...
  Future<DataState<bool>> createFeedback({
    ...
    bool isAnonymous = false,
      ...
      guard(() async {
        ...
        final raw = res.data as Map<String, dynamic>;
        if (raw['success'] != true) {
          throw AppException(message: (raw['message'] as String?) ?? 'Failed to submit feedback');
        }
        return true;
      });
  ...
  Future<DataState<bool>> updateFeedback({
    ...
    String? comment,
      ...
      guard(() async {
        ...
        final raw = res.data as Map<String, dynamic>;
        if (raw['success'] != true) {
          throw AppException(message: (raw['message'] as String?) ?? 'Failed to update feedback');
        }
        return true;
      });

  @override
  Future<DataState<List<PendingFeedbackRequest>>> getPendingFeedbackRequests() => guard(() async {
        final res = await _service.getPendingFeedbackRequests();
        final raw = res.data as Map<String, dynamic>;
        if (raw['success'] != true) {
          throw AppException(message: (raw['message'] as String?) ?? 'Failed to fetch pending requests');
        }
        return (raw['data'] as List<dynamic>)
            .map((e) => PendingFeedbackRequest.fromJson(e as Map<String, dynamic>))
            .toList();
      });
}
